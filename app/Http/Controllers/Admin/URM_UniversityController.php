...
class URM_UniversityController extends Controller
{
    public function index (Request $request)
    {
        if(UserCan('application.create')){
            $university_list = University::select('id', 'name', 'urm_name')
            ->whereNotNull('urm_name')
            ->where('urm_name', '!=', '')
            ->whereNotNull('urm_contact_no')
            ->where('urm_contact_no', '!=', '')
            ->get()
            ->toArray();
            return view('admin-panel.urm_university.index', compact('university_list'));
        }else{
    ...
    public function fetch(Request $request)
{
    ...
    if ($request->has('university_id') && !empty($request->university_id)) {
        $query->where('universities.id', $request->university_id);
    }
    if ($request->has('urm_name') && !empty($request->urm_name)) {
        $query->where('universities.urm_name', $request->urm_name);
    }
    ...
    $universities = $query->orderBy('universities.id', 'desc')
        ->paginate($limit, ['id', 'name', 'urm_name', 'urm_contact_no', 'logo'], 'page', $currentPage);

    // Render the view with paginated results
    $html = view('admin-panel.urm_university.detail-part.detail-fetch', ['university_list' => $universities])->render();
    $pagination = view('admin-panel.urm_university.detail-part.pagination', [
        'totalPage' => $universities->lastPage(),
        'CurrentPage' => $universities->currentPage(),
        'limit' => $limit
    ])->render();
    ...
    {   
        ...
        if($application){
            ...
            return view('admin-panel.urm_university.detail-part.detail', compact('course','application'));
        }else{
    ...
    public function studentInfo(Request $request){
        ...
        return view('admin-panel.urm_university.detail-part.student-information',compact('application_id','student_info'));
    }
    ...
    public function studentDocument(Request $request){
        ...
        return view('admin-panel.urm_university.detail-part.student-document', compact('application_id','documents_list'));
    }
