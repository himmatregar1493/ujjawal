                        ...
                        <tr>
                            ...
                            <td style="width:60%">
                                <select name="vegitable" id="vegitable" class="form-control">
                                    <option value="" class="vegitable custom-select" disabled selected>Select Products</option>
                                    @foreach($productList as $product)
                                        <option value="{{ $product['id'] }}" data-price="{{ $product['price'] }}" data-remaining_quantity="{{ $product['remaining_quantity'] }}" data-product_unit="{{ $product['product_unit'] }}" data-product_id="{{ $product['id'] }}"  class="vegitable custom-select">
                                            {{ $product['name'] }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <td><p id="add" class="btn btn-primary">Add</p></td>
                        </tr>
                    </tbody>
                </table>
...
$(document).ready(function() {
    ...
    $('#product_detail_form').submit(function(e) {
        ...
        formData.append('date', datetime);
            ...
            error: function(xhr) {
                ...
                Swal.fire({
                    title: 'Error!',
                    text: xhr.responseJSON ? xhr.responseJSON.message : 'An unknown error occurred. Please contact the developer.',
                    icon: 'error',
                    confirmButtonText: 'OK'
                });
            }
    ...
    $('#vegitable').change(function() {
        ...
        $.ajax({
            ...
            error: function(xhr) {
                $('#errorMsg').text('An error occurred while fetching product details.').show();
            }
        });
    ...
    $('#add').click(function() {
        ...
        if (!product_id || qty <= 0 || price <= 0) {
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Please select a product and enter valid quantity and price.'
            });
            return;
        }
        ...
        if (qty > remainingQuantity) {
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Entered quantity exceeds the remaining quantity.'
            });
            return;
        }
    ...
    function attachRowEvents() {
        $('#new').find('.price, .quantity, .discount, .cgst_rate, .sgst_rate, .igst_rate').on('input change', function() {
    ...
    window.validateQuantity = function(el) {
        ...
        // Check if entered quantity exceeds remaining quantity
        if (enteredQuantity > remainingQty) {
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Entered quantity exceeds the available quantity.'
            });
            $(el).val(remainingQty); // Reset the quantity to remaining quantity
        }
