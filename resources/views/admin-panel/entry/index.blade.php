                ...
                <div class="col-xl">
                    <div class="card mb-4">
                        <div class="card-header d-flex justify-content-between align-items-center pb-2 p-2 pt-3">
                            <h5 class="mb-0">Entry List</h5>
                            @php
                                $companyId = getCompanyId();
                            @endphp

                            @if(is_numeric($companyId) && UserCan('entry.create'))
                                <a href="{{ route('entry.create') }}">
                                    <button id="create-client-btn" class="mb-0 btn btn-primary">Create</button>
                                </a>
                            @endif
                        </div>
    ...
    <script>
        $(document).ready(function() {
            function loadRoles(filter = null,var_name = null) {
                ...
                $.ajax({
                    url: '{{ route('entry.fetch') }}', // Adjust this route as needed
                    type: 'GET',
                    data: formData,
                    success: function(response) {
                         $('#clients_table').html(response.html);
            ...
            function CountData() {
                ...
                $.ajax({
                    url: '{{ route('entry.CountData') }}', // Adjust this route as needed
                    type: 'GET',
                    success: function(response) {
