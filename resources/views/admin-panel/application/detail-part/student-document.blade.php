            ...
            <ul id="uploaded-docs-list" class="list-unstyled">
                @foreach($documents_list as $key => $value)
                   ...
                   <a href="{{ asset('admin_assets\images\student_documents') }}/{{ $value['stored_name']}}" target="_blank"
                        class="file-name">{{ $value['original_name']}}</a><button class="btn btn-primary">Remove</button></li>
                @endforeach
            </ul>
        </div>
    </div>
    ...
    function uploadFiles() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                GetStudentDocument(); // Assuming this function refreshes the student information on the page
            },
            error: function(xhr) {
                Swal.close();
