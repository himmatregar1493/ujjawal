<style>
    ...
    @if (count($application_stageses) <= 0) 
    ...
    <div class="detail-table">
        <table class="table">
            ...
            No record Found
                    ...
                    @else
                    @foreach ($application_stageses as $item)
                    <tr>
                        <td>{{ $item['id'] }}</td>
                        <td>{{ $item['name'] }}</td>
    ...
    <script>
        function change_status(element_id, id) {
            ...
            $.ajax({
                url: '{{ route('comment.change_status') }}', // Adjust this route as needed
                type: 'POST'
                , data: formData
