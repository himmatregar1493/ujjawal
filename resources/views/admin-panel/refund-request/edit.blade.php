            ...
            <div class="col-xl">
                <div class="card mb-4">
                    <div class="card-header d-flex justify-content-between align-items-center pb-2 p-2 pt-3">
                        <h5 class="mb-0">Refund Request Edit</h5>
                        <a href="{{ route('refund-request.index') }}">
                            <button id="create-RefundRequest-btn" class="mb-0 btn btn-primary">Back</button>
                        </a>
                    </div>
                    <div class="card-body pb-2 p-2 pt-3">
                        <form id="apply-course-form" method="POST" action="{{ route('refund-request.update', $RefundRequest->id) }}" enctype="multipart/form-data">
                            @csrf
                            {{-- @method('PUT') --}}
                            <div class="row">
                                <!-- Refund Request Name Input -->
                                <div class="mt-1 mb-1 col-12 col-md-6 col-lg-4 col-xl-4">
                                    <label class="form-label required-label" for="RefundRequest">Refund Request Name</label>
                                    <input type="text" name="name" id="RefundRequest" class="form-control" placeholder="Enter Refund Request name..." value="{{ old('name', $RefundRequest->name) }}">
                                    @error('name')
                                ...
                                <div class="mt-1 mb-1 col-12 col-md-6 col-lg-4 col-xl-4">
                                    ...
                                    <select class="form-control select2" id="university_id" name="university_id">
                                        ...
                                        <option value="{{ $university['id'] }}" {{ old('university_id',$RefundRequest['university_id']) == $university['id'] ? 'selected' : '' }}>{{ $university['name'] }}</option>
                                        @endforeach
                                    </select>
                                    @error('university_id')
                                    <div class="text-danger mt-1">{{ $message }}</div>
                                ...
                                <div class="mt-1 mb-1 col-12 col-md-6 col-lg-4 col-xl-4">
                                    ...
                                    <div id="file-preview" class="file-preview mt-3">
                                        @if(pathinfo($RefundRequest->link, PATHINFO_EXTENSION) === 'pdf')
                                            <embed src="{{ asset('admin_assets/images/RefundRequest_image/' . $RefundRequest->link) }}" class="pdf-preview" type="application/pdf">
                                        @elseif(in_array(pathinfo($RefundRequest->link, PATHINFO_EXTENSION), ['doc', 'docx']))
                                            <div class="doc-preview">
                                                <a href="{{ asset('admin_assets/images/RefundRequest_image/' . $RefundRequest->link) }}" target="_blank" class="btn btn-secondary">View Document</a>
                                            </div>
                                        @else
                                            <img src="{{ asset('admin_assets/images/RefundRequest_image/' . $RefundRequest->link) }}" class="img-preview">
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="mt-1 mb-1 col-12 col-md-6 col-lg-4 col-xl-4">
                                    ...
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" id="status" name="status" value="1" {{ old('status', $RefundRequest->is_active) ? 'checked' : '' }}>
                                    </div>
                                    @error('status')
                                    <div class="text-danger mt-1">{{ $message }}</div>
                                    @enderror
